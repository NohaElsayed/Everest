       ...
       <div class=" inline_product clickable d-flex justify-content-center"
           ...
           <div class="d-flex d-block" style="cursor: pointer;">
               <a href="{{route('product',$product->slug)}}">
                   <img src="{{\App\CPU\ProductManager::image_path('thumbnail')}}/{{$product['thumbnail']}}"
                        onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'"
                        style="width: 100%;border-radius:5px 5px 0px 0px;height:157px !important;">
               </a>
           </div>
       </div>
       <div class="single-product-details " style="position:relative;height:80px;padding-top:10px;border-radius:0px 0px 5px 5px; ">
           <div class="text-{{Session::get('direction') === "rtl" ? 'center pr-3' : 'center pl-3'}}">
               <a style="text-align: center !important;" href="{{route('service',$product->slug)}}">
                   {{ Str::limit($product['name'], 23) }}x
               </a>
           </div>
...
{{--                @for($inc=0;$inc<5;$inc++)--}}
                ...
                {{--                <label class="badge-style">( {{$product->reviews_count}} )</label>--}}
                    <label class="badge-style">{{$product->phone}}</label>
            </span>
           </div>
