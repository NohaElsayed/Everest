        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{route('admin.business-settings.shipping-cats.add')}}"
                            ...
                            <div class="form-group">
                                <div class="row justify-content-center">
                                    <div class="col-md-12">
                                        <label for="duration">{{\App\CPU\translate('Type')}}</label>
                                        <select name="type" class="form-control">
                                            <option value="1">{{\App\CPU\translate('Amount')}}</option>
                                            <option value="2">{{\App\CPU\translate('Percent')}}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="row justify-content-center">
                                    <div class="col-md-12">
                                        <label for="amount">{{\App\CPU\translate('Amount')}}</label>
                                        <input type="number" min="0" max="1000000" name="amount" class="form-control"
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" width="100%" cellspacing="0"
                                ...
                                <tr>
                                    ...
                                    <th scope="col">{{\App\CPU\translate('type')}}</th>
                                    <th scope="col">{{\App\CPU\translate('amount')}}</th>
                                    <th scope="col" style="width: 50px">{{\App\CPU\translate('action')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($shipping_cats as $k=>$method)
                                    <tr>
                                        <th scope="row">{{$k+1}}</th>
                                        <td>{{$method['title']}}</td>
                                        <td>
                                            {{$method['type'] == 1?\App\CPU\translate('Amount'):\App\CPU\translate('Percent')}}
                                        </td>
                                        <td>
                                            @if ($method['type'] == 1)
                                            {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($method['amount']))}}
                                            @else
                                                {{ $method['amount'] }} %
                                            @endif
                                        </td>
                                            ...
                                            <div class="dropdown float-right">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.business-settings.shipping-cats.edit',[$method['id']])}}">{{\App\CPU\translate('Edit')}}</a>
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.business-settings.shipping-cats.delete')}}",
                        method: 'POST',
                        data: {id: id},
                        success: function () {
